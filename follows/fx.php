<?php
	...
	require CODEDIR.'twitterusers.class.php';
	require CODEDIR.'followers.class.php';
	...
	function & GetDB()
	{
		...
		if ($_db === false)
		{
			$_db = mysql_connect($GLOBALS['_db']['host'], $GLOBALS['_db']['username'], $GLOBALS['_db']['password']) or die('DB gone? <!-- '.mysql_error().' -->');
			mysql_select_db($GLOBALS['_db']['database'], $_db) or die('DB dead? <!-- '.mysql_error().' -->');
