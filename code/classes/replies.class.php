<?php
	...
	class Replies
	{
			...
			else
			{
				...
				else
				{
					...
					if ($user['last_id'] == 0)
					{
						...
						$replies_json = $twitter->OAuthRequest('https://api.twitter.com/1/statuses/mentions.json', array('count' => 1), 'GET');
					...
					else
					{
						...
						$response = $twitter->OAuthRequest('https://api.twitter.com/1/statuses/mentions.json', array('count' => 200, 'since_id' => $user['last_id']), 'GET');
						if ($twitter->lastStatusCode() != 200)
						{
							switch ($twitter->lastStatusCode())
							{
			...
			else
			{
				...
				else
				{
					...
					else
					{
						...
						$mail->IsSMTP();
						$maildetails = __('smtp', 'host');
						var_dump($maildetails);
		...
		static private function GetNextUser()
		{
			...
			//$sql = 'update `'.self::TABLE_USERS.'` set `processor_pid` = "'.$pid.'" where email = "dev@stut.net"';
